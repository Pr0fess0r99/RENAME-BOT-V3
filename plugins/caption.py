...
async def add_caption(client, message):
    ...
    await message.reply_text("**Your Caption Successfully Added ☑️**")

@Client.on_message(filters.private & filters.command('del_caption'))
async def delete_caption(client, message): 
    caption = find(int(message.chat.id))[1]
    if not caption:
        await message.reply_text("**Your Don't Have Any cusytom captyion ✖️**")
        return
...
async def see_caption(client, message): 
    ...
    if caption:
       ...
       await message.reply_text("**Your Don't Have Any Custom Caption ✖️**")
