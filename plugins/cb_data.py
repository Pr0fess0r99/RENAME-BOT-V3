...
async def rename(bot,update):
	...
	await update.message.delete()
	await update.message.reply_text(f"**Please Enter The New Filename🔸Or Copy Paste Old File Name**\n\n**✍️ Note:- .mp4 Extension is required**",reply_to_message_id = id,
...
async def vid(bot, update):
    ...
    try:
        path = await bot.download_media(
            message=file, progress=progress_for_pyrogram, progress_args=("``` Trying To Download...```", ms, c_time)
        )
    ...
    if value < file.file_size:
        ...
        try:
            filw = await app.send_video(
                log_channel, video=file_path, thumb=ph_path, duration=duration, caption=caption,
        ...
        try:
            await bot.send_video(
                update.from_user.id, video=file_path, thumb=ph_path, duration=duration, caption=caption,
                progress=progress_for_pyrogram, progress_args=("```Trying To Uploading```", ms, c_time)
            )
            await ms.delete()
            os.remove(file_path)
